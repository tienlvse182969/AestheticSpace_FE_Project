...
type Errors = { identifier?: string; username?: string; email?: string; password?: string };
...
export function AuthPage() {
  ...
  const handleSubmit = () => {
    ...
    if (isLogin) {
      ...
      else if (identifier.includes("@") && !isEmailFormat(identifier))
    ...
    if (isLogin) {
      if (isEmailFormat(identifier)) {
        displayName  = identifier.split("@")[0].replace(/[._]/g, " ").replace(/\b\w/g, c => c.toUpperCase());
        displayEmail = identifier.trim();
      } else {
        displayName  = identifier.trim().replace(/\b\w/g, c => c.toUpperCase());
        displayEmail = "";
      }
    } else {
    ...
    sessionStorage.setItem("user", JSON.stringify({ name: displayName, email: displayEmail }));
    navigate("/space");
  };
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <Box as="button" w="full" h="44px" borderRadius="6px" color="white" fontSize="sm"
              letterSpacing="0.06em" cursor="pointer" border="none" transition="all 0.2s" mb={5}
              style={{ background: "linear-gradient(135deg, #1a3a8a 0%, #1e4db5 100%)", boxShadow: "0 4px 16px rgba(26,58,138,0.4)" }}
              _hover={{ transform: "translateY(-1px)", boxShadow: "0 6px 20px rgba(26,58,138,0.55)" }}
